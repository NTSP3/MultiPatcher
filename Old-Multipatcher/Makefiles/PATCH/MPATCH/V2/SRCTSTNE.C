...
/*
  ...
  if (yn13 == 13) { // Run statements if a given condition is true
...
*/
  ...
  int key;
  while (1) { // Runs an indefinite loop until a condition is met
    if (kbhit()) {  // Don't execute till the person presses a key
      key = getch();  // Save the key pressed by the user in a variable
      if (key == 0x3D) {  // Run statements if a given condition is true
//	textbackground(LIGHTGRAY);
//	textcolor(RED);
//	cprintf("F3 key อ is pressed!\n");
	UserQuitCheck();
	break;
      }
